# This is the program for the Assignment1 Task 2, it contains rNGeneration function, saveToFile function and insertionSort functon. This program is used for sorting a series numbers that are generated by rNGenerator by using insertion sorting algorithm and save the result into a file.
...
def insertionSort(_resLis): # The function to sort the list generated by the rNGnerator by insertion sort algorithm
   for i in range(1,len(_resLis)):
      current = _resLis[i]
      j = i
      while j > 0 and _resLis[j-1] > current:
         _resLis[j] = _resLis[j-1]
         j -= 1
         _resLis[j] = current
   return _resLis

resListUnsort = rNGenerator(lowerLimit,upperLimit,numOfValues,noDuplicates) # Generate a list of random number
if not resListUnsort == None: # For the validation
